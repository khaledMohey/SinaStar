...
def home(request):
    today_date = now().date()
    orders_today = Order.objects.filter(created_at__date=today_date)
    total_orders_today = orders_today.count()
    total_sales_today = sum(o.total for o in orders_today)
    unpaid_orders_today = orders_today.filter(is_paid=False).count()

    pending_items_count = OrderItem.objects.filter(
        is_done=False,
        order__created_at__date=today_date
    ).count()

    low_stock_count = SinastarInventory.objects.filter(
        minimum_stock__gt=0,
        addition__lte=models.F("minimum_stock")
    ).count()
...
def orders_list(request):
    ...
    paid_count = all_orders.filter(is_paid=True).count()
    unpaid_count = all_orders.filter(is_paid=False).count()
    total_tax = sum(order.tax for order in all_orders)
    total_discount = sum(order.discount for order in all_orders)
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                ...
                # Recalculate discount for Qeta3 orders
                if order.order_type == 'qeta3' and order.officer:
                    order.discount = order.subtotal * order.officer.discount_rate
                
                # Update tax (only for non-takeaway orders)
                try:
...
def daily_closing(request):
    ...
    # 🟢 إجمالي كل قسم + صافي اليوم (إضافة التبس وخصم النسريات)
    section_codes = [
        ("barista", "بوفيه/باريستا"),
        ("mat3am", "مطعم"),
        ("canteen", "كانتين"),
        ("shesha", "شيشة"),
    ]
    ...
    section_totals = {}
    for code, label in section_codes:
        sec_total = (
            order_items.filter(menuitem__section=code)
            .aggregate(val=Sum(F("menuitem__price") * F("quantity")))["val"]
            or 0
        )
        section_totals[label] = sec_total

    gross_total = sum(section_totals.values())
    net_total = gross_total + total_tips - total_nesrayat
